...
5. **Never trigger the $80 shutdown function** for testing. It is live —
  `SIMULATE_DEACTIVATION = False` in `src/backend/services/billing-shutdown/main.py`.
...
| What | Where |
|---|---|
| Collector application | `src/backend/services/collector/` |
| Billing shutdown function | `src/backend/services/billing-shutdown/` |
| Website and tracking scripts | `src/frontend/` |
| Infrastructure | `infra/terraform/` (this directory) |
